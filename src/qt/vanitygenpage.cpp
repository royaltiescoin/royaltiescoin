    ...
    ui(new Ui::VanityGenPage)
{
    ...
    ui->tableView->setStyleSheet("QTableView{background-color:#0e2947;alternate-background-color:#1b3e66;color:white;}");
    ...
    ui->lineEdit->setValidator(new QRegExpValidator(QRegExp("[R]{1,1}[YZa-w]{1,1}[1-9A-HJ-NP-Za-km-z]{10,10}"), NULL));
...
#ifndef Q_OS_MAC
                gui->trayIcon->showMessage("RoyaltiesCoin: VanityGen",
...
void VanityGenPage::checkAllowedText(int curpos)
{
    ...
    ui->lineEdit->setValidator(new QRegExpValidator(QRegExp("[R]{1,1}[YZa-w]{1,1}[1-9A-HJ-NP-Za-km-z]{14,14}"), NULL));
    QChar secondChar;
    if(ui->lineEdit->text().length() > 1){
        secondChar = ui->lineEdit->text().at(1);
    }
    if(curpos == 0){
        ui->labelAllowed->setText("Allowed(@"+QString::number(curpos)+"): R");
    } else if(curpos == 1){
        ui->labelAllowed->setText("Allowed(@"+QString::number(curpos)+"): YZabcdefghijkmnopqrstuvw");
    } else if(curpos == 2){
        if(secondChar == 'Y'){
            ui->lineEdit->setValidator(new QRegExpValidator(QRegExp("[R]{1,1}[YZa-w]{1,1}[c-km-z]{1,1}[1-9A-HJ-NP-Za-km-z]{13,13}"), NULL));
            ui->labelAllowed->setText("Allowed(@"+QString::number(curpos)+"): cdefghijkmnopqrstuvwxyz");
        } else if(secondChar == 'w'){
            ui->labelAllowed->setText("Allowed(@"+QString::number(curpos)+"): 123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwx");
            ui->lineEdit->setValidator(new QRegExpValidator(QRegExp("[R]{1,1}[YZa-w]{1,1}[1-9A-HJ-NP-Za-km-x]{1,1}[1-9A-HJ-NP-Za-km-z]{13,13}"), NULL));
        } else{
